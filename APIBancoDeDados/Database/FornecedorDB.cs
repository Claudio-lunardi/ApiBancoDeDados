...
namespace APIBancoDeDados.Database
{
    internal class FornecedorDB
    {
        ...
        internal void deletar(string fornecedorModel)
        {
            string queryString = ("delete from Fornecedores where cnpj = @cnpj");
            SqlCommand command = new SqlCommand(queryString, connection);

            connection.Open();

            command.Parameters.AddWithValue("@cnpj", fornecedorModel);
